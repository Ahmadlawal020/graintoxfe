    ...
    <div className="flex flex-col h-full bg-[#161a1e] text-[11px] font-mono select-none">
      ...
      <div className="px-3 py-2.5 border-b border-gray-800 flex justify-between items-center shrink-0">
        ...
        <div className="flex gap-1">
          <button className="w-5 h-4 rounded-sm bg-gradient-to-b from-red-500/60 to-primary/60 border border-gray-700" title="Both" />
          <button className="w-5 h-4 rounded-sm bg-primary/30 border border-gray-800" title="Bids" />
      ...
      <div className="flex-1 overflow-auto min-h-0 flex flex-col">
        ...
        <div className="flex flex-col-reverse mt-auto">
          {asks.map((ask, i) => (
            <div key={`ask-${i}`} className="grid grid-cols-3 py-[3px] px-3 hover:bg-white/[0.03] relative cursor-pointer items-center transition-colors">
              ...
              <span className="text-right text-muted-foreground relative z-10 text-[9px]">{(ask.total / 1000).toFixed(1)}K</span>
            </div>
          ))}
        </div>
        ...
        <div className="py-2 px-3 border-y border-gray-800/60 bg-[#1e2329]/50 shrink-0 my-0.5">
          <div className="flex items-baseline gap-2">
            <span className="text-lg font-bold text-primary tracking-tighter">{currentPrice.toLocaleString()}</span>
        ...
        <div className="flex flex-col">
          {bids.map((bid, i) => (
            <div key={`bid-${i}`} className="grid grid-cols-3 py-[3px] px-3 hover:bg-white/[0.03] relative cursor-pointer items-center transition-colors">
              ...
              />
              <span className="text-primary/80 relative z-10 font-semibold">{bid.price.toLocaleString()}</span>
              <span className="text-right text-muted-foreground relative z-10">{bid.amount.toFixed(3)}</span>
              <span className="text-right text-muted-foreground relative z-10 text-[9px]">{(bid.total / 1000).toFixed(1)}K</span>
            </div>
          ))}
        </div>
      </div>
