  ...
  return (
    <div className="w-full h-full bg-card flex flex-col font-sans">
      <div className="p-4 space-y-4 bg-accent/10">
        ...
        <div className="grid grid-cols-3 text-[9px] text-muted-foreground uppercase font-black tracking-widest px-1">
          <span>Asset</span>
        ...
        {filtered.map((asset) => (
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-foreground font-bold tracking-tight">{asset.symbol}</span>
            </div>
            <div className="flex flex-col items-end justify-center">
              <span className="text-[11px] text-muted-foreground font-mono font-bold tracking-tighter">{asset.price.toLocaleString()}</span>
              <span className="text-[8px] text-muted-foreground font-bold uppercase">NGN</span>
            </div>
            <div className="flex items-center justify-end">
              <span className={`text-[10px] font-black px-1.5 py-0.5 rounded-sm ${asset.change >= 0 ? "text-primary bg-primary/10" : "text-red-500 bg-red-500/10"}`}>
